...
The database schema is defined in `src/main/resources/db/migration`. The first migration creates customers, products, orders, and order items. Orders reference customers, and order items reference both orders and products.
...
5. Uses JDBC to print the customer count.

The seeder is intended for a fresh development database. It creates deterministic SKU values and assumes the newly inserted customer and product IDs begin at one, so rerunning it against existing data can violate unique constraints or produce incorrect relationships.
...
`DB.get()` opens a JDBC connection using the resolved configuration. `CustomerDao` demonstrates the intended DAO pattern:
...
The starter does not add an ORM or DI container. The goal is to make the connection, transaction, and SQL boundaries visible.
...
4. Unit tests validate `Env`, `Seeder` input validation, and data-mask confirmation parsing without a database.
5. Integration tests validate the schema, DAO CRUD, seeding, masking, and foreign-key relationships against MySQL.
...
- Seeding reports duplicate SKU or email errors: use a fresh development database, or add an explicit reset/cleanup workflow before reseeding.
